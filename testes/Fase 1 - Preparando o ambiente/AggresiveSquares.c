...
int main(){
	...
	ALLEGRO_DISPLAY* disp = al_create_display(1000, 600);							//Cria uma janela para o programa, define a largura (x) e a altura (y) da tela em píxeis (320x320, neste caso)
	...
	while(1){																		//Laço principal do programa
		...
		if (event.type == 30){														//O evento tipo 30 indica um evento de relógio, ou seja, verificação se a tela deve ser atualizada (conceito de FPS)
			if (flag > 10) {
				al_clear_to_color(al_map_rgb(0, 255, rand()));				// Substitui tudo que estava desenhado na tela por um fundo preto
				flag++;
				flag = 0;
			} else {
